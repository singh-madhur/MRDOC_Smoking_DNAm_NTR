...
twin_dat <- twin_dat_long |>
  select(FamilyNumber, zygo, Twin_nr, fisnumber, age, female, formerSmk) |>
  pivot_wider(id_cols = c(FamilyNumber, zygo), names_from = Twin_nr,
              values_from = c(fisnumber, age, female, formerSmk),
              names_sep = "_")
...
vars      <- 'formerSmk'                  # list of variables names
...
hetcor(mzDataF$formerSmk_1, mzDataF$formerSmk_2)$cor

hetcor(dzDataF$formerSmk_1, dzDataF$formerSmk_2)$cor

# Set Starting Values
svBe      <- 0.01                      # start value for regressions
svTh      <- .2                        # start value for thresholds
svCor     <- .7                        # start value for correlations
...
modelSAT  <- mxModel( "formerSmkSAT", pars, modelMZ, modelDZ, multi, ciCor, ciThre )
...
# Set Starting Values
svPa      <- .7                        # start value for A 
svPc      <- .1                        # start value for C
svPe      <- .2                        # start value for E
...
modelACE  <- mxModel( "formerSmkACE", pars, var1, modelMZ, modelDZ, multi, estUS, ciACE )
